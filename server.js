...
// Main Menu
function mainMenu() {
    inquirer.prompt(prompts.mainMenuPrompt).then(function(task) {
        switch (task.menu) {
...
function addDepartment() {
    inquirer.prompt(prompts.addDepartmentPrompt).then(function (answer) {
...
function addRole() {
    ...
    connection.query(query, function (err, res) {
        ...
        prompts.addRolePrompt[2].choices = departments;
        inquirer.prompt(prompts.addRolePrompt).then(function (answer) {
...
function addEmployee() {
    ...
    connection.query(query, function (err, res) {
        ...
        prompts.addEmployeePrompt[2].choices = roles;
        ...
        connection.query(query, function (err, res) {
            ...
            prompts.addEmployeePrompt[3].choices = employees;
            inquirer.prompt(prompts.addEmployeePrompt).then(function (answer) {
...
function updateEmployeeRole() {
    ...
    connection.query(query, function (err, res) {
        let roles = res.map(function(x) { return {name: x.title, value: x.id} });
        prompts.updateEmployeeRolePrompt[1].choices = roles;
        ...
        connection.query(query, function (err, res2) {
            ...
            prompts.updateEmployeeRolePrompt[0].choices = employees;
            inquirer.prompt(prompts.updateEmployeeRolePrompt).then(function (answer) {
